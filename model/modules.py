...
# Version of the `embedding_module_log` with sin & cos functions hardcoded
class sin_cos_embedder(nn.Module):
    def __init__(self, num_freqs=20, max_freq=10, ch_dim=1, include_in=True):
        super().__init__()
        self.freqs = torch.nn.Parameter(2.0**torch.from_numpy(np.linspace(start=0.0,stop=max_freq, num=num_freqs).astype(np.single)), requires_grad=False)
        self.ch_dim = ch_dim
        self.include_in = include_in
    ...
    def forward(self, x_input):
        if self.include_in:
            ...
            out_list = []
        for freq in self.freqs:
            out_list.append(torch.sin(x_input*freq))
        for freq in self.freqs:
            out_list.append(torch.cos(x_input*freq))
        return torch.cat(out_list, dim=self.ch_dim)

...
class kernel_residual_fc_embeds(nn.Module):
    def __init__(self, input_ch, intermediate_ch=512, grid_ch = 64, num_block=8, output_ch=1, grid_gap=0.25, grid_bandwidth=0.25, bandwidth_min=0.1, bandwidth_max=0.5, float_amt=0.1, min_xy=None, max_xy=None, probe=False):
        ...
        # (TW) Workaround for Tensor Script not supporting non-literal getattr()
        # So replace getattr(self, "left_right_{}".format(k)) with self.left_right_array[k + 1]
        self.left_right_array = []
        for k in range(num_block - 1):
            name = "left_right_{}".format(k)
            self.register_parameter(name, nn.Parameter(torch.randn(1, 1, 2, intermediate_ch)/math.sqrt(intermediate_ch),requires_grad=True))
            self.left_right_array.append(getattr(self, name))
        ...
        for k in range(4):
            ...
            self.rots_array.append(getattr(self, name))
    ...
    def forward(self, input_stuff, rot_idx, sound_loc):
        SAMPLES = input_stuff.shape[1]
        sound_loc_v0 = sound_loc[..., :2]
        sound_loc_v1 = sound_loc[..., 2:]

        # Prevent numerical issues
        self.bandwidths.data.set_(torch.clamp(self.bandwidths.data, self.bandwidth_min, self.bandwidth_max))
        ...
        # Was: torch.stack([getattr(self, "rot_{}".format(rot_idx_single)) for rot_idx_single in rot_idx], dim=0)
        # Wchich passes to "stack()" a single-element list of Tensors...equal to just rot[rot_idx]
        # Not sure what's the point of it being so convoluted originally
        rot_latent = torch.stack([self.rots_array[rot_idx]], dim=0)

        out = self.proj(my_input).unsqueeze(2).repeat(1, 1, 2, 1) + getattr(self, "left_right_0") + rot_latent[:, 0]
        
        for k, layer in enumerate(self.layers):
            out = layer(out) + self.left_right_array[k + 1] + rot_latent[:, k + 1]
